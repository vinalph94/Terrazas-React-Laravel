...
const PoolManagerEvent = () => {
  ...
  async function fetchData() {
    axios
      .get("/api/poolmanager/Pool_Event_Read.php")
      .then((response) => {
  ...
  const handleDelete = async(id) => {
    try{
      console.log(id);
      const response= await axios.post('/api/poolmanager/Pool_event_Delete.php', { id })
      console.log(response);
      fetchData();
      } 
      catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "/api/poolmanager/Pool_event_Insert.php",
        data
      );
      e.preventDefault();
      fetchData();
...
export default PoolManagerEvent;

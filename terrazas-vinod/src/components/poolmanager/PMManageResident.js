...
function PMManageResident() {
  const [AllResident, setAllResident] = useState([]);
  const [Request, setRequest] = useState([]);

  async function fetchData() {
    axios
      .get(BASE_URL+"poolmanager/Pool_Access_Read.php?filter='Resident'")
      .then((response) => {
  ...
  async function fetchDataRequest() {
    axios
      .get(BASE_URL+"poolmanager/Pool_Access_Request.php?filter='Resident'")
      .then((response) => {
        console.log(response);
  ...
  const handleDelete = async(id) => {
    try{
      console.log(id);
      const response= await axios.post(BASE_URL+'poolmanager/Pool_Access_Delete.php', { id })
      console.log(response);
      fetchDataRequest();
      fetchData();
      } 
  ...
  const handleAccept = async(id) => {
    try{
      console.log(id);
      const response= await axios.post(BASE_URL+'poolmanager/Pool_Access_Accept.php', { id })
      console.log(response);
      fetchDataRequest();
      fetchData();
      } 
...
export default PMManageResident;
